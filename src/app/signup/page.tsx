      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-8">
            <div className="text-center mb-6">
              ...
              <h1 className="text-2xl font-bold tracking-tight text-slate-950">
                Register School Station
              </h1>
              <p className="mt-1 text-xs sm:text-sm text-slate-500">
                Provision your official school examination center account for autonomous hall monitoring.
              </p>
            </div>
            ...
            <form action={formAction} className="space-y-4">
              ...
              <div>
                <label
                  ...
                  className="block text-xs font-medium text-slate-700 mb-1.5"
                >
                  School Code Prefix (e.g. UB, YALE, GBHS)
                </label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400 text-xs font-mono font-bold">
                    ID
                  </span>
                  <input
                    id="codePrefix"
                    ...
                    className="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0e5a4d]/30 focus:border-[#0e5a4d] transition-colors uppercase font-mono"
                  />
                </div>
              </div>
              ...
              {state.error && (
                ...
                >
                  <p className="font-semibold mb-0.5">Registration Notice</p>
                  <p>{state.error}</p>
                </div>
              )}
              ...
              {state.success && (
                <div
                  role="status"
                  className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-800 leading-relaxed"
                >
                  <p className="font-semibold mb-0.5">Registration Successful</p>
                  <p>{state.success}</p>
                  <div className="mt-2">
                    <Link
                      href="/login"
                      className="font-semibold text-emerald-950 underline hover:text-emerald-800"
                    >
              ...
              >
                {isPending ? (
                  <>
                    ...
                    Creating School Station…
                  </>
                ) : (
                  <>
                    Create School Station
                    <ArrowRightIcon />
                  </>
                )}
              </button>
          ...
          <div className="mt-4 text-center text-[11px] text-slate-400 flex items-center justify-center gap-2">
            ...
            <span>•</span>
            <span>Ministry of Secondary Education Protocol</span>
          </div>
        </div>
      </main>
    </div>
